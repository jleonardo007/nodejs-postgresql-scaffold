...
export const lintstaged = `export default {
  ...
  '*.{ts,js,cjs,mjs,json,md,yml,yaml}': (filenames) => [
...
export const dockerfileNpm = `# Stage 1: Base — npm
...
export const dockerfileYarn = `# Stage 1: Base — yarn
FROM node:24-alpine AS base
WORKDIR /app
...
export const dockerfilePnpm = `# Stage 1: Base — pnpm
FROM node:24-alpine AS base
WORKDIR /app
...
export const biomeLintStaged = `export default {
  "*.{ts,js,cjs,mjs,json,md,yml,yaml}": (filenames) => [
...
export default defineConfig({
  test: {
    ...
    environment: 'node',
    root: '.',
    include: ['tests/**/*.ts', 'src/**/*.ts', '**/__tests__/**/*.ts', '**/?(*.)+(spec|test).ts'],
    coverage: {
      provider: 'v8',
      include: ['src/**/*.ts'],
      exclude: [
...
export const pnpmConfig = () => {
  ...
  const pnpmConfigWithEslintPrettier = `# Hoist ESLint and its plugins to root node_modules so they are accessible as CLI tools
publicHoistPattern:
- '*eslint*'
- '*prettier*'
- "*types*"

# Do not fail on peer dependency version conflicts
strictPeerDependencies: false
`;

  const pnpmConfigWithBiome = `# Hoist ESLint and its plugins to root node_modules so they are accessible as CLI tools
publicHoistPattern:
  - '*types*'
...
`;
  ...
  if (flags.linter === 'eslint') {
    return pnpmConfigWithEslintPrettier;
  } else {
    return pnpmConfigWithBiome;
  }
};
