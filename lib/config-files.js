export function setPackageJson({
  ...
  const databaseScripts = addDocker
    ? {
        'docker:exec': 'dotenv -e .env -- bash -c \'docker exec -it ${PROJECT_NAME} "$@"\' --',
        'db:drop':
          'npm run docker:exec -- npm run typeorm -- schema:drop -d src/database/data-source.ts',
        'db:sync':
          'npm run docker:exec -- npm run typeorm -- schema:sync -d src/database/data-source.ts',
        'db:reset':
          'npm run docker:exec -- npm run db:drop && npm run docker:exec -- npm run migration:run && npm run docker:exec -- npm run seed',
        'migration:generate':
          'npm run docker:exec -- npm run typeorm -- migration:generate src/database/migrations/${npm_config_name} -d src/database/data-source.ts',
        'migration:create':
          'npm run docker:exec -- npm run typeorm -- migration:create src/database/migrations/${npm_config_name}',
        'migration:run':
          'npm run docker:exec -- npm run typeorm -- migration:run -d src/database/data-source.ts',
        'migration:revert':
          'npm run docker:exec -- npm run typeorm -- migration:revert -d src/database/data-source.ts',
        'migration:show':
          'npm run docker:exec -- npm run typeorm -- migration:show -d src/database/data-source.ts',
        seed: 'ts-node -r tsconfig-paths/register src/scripts/seeds.ts',
        'seed:run': 'npm run docker:exec -- npm run seed',
      }
    : {
        'migration:generate':
