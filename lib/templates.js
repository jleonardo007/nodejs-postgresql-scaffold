...
export const databaseCliCmd = () => {
  ...
  export function drop(exec: ExecFn): void {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm -- schema:drop -d src/database/data-source.ts\`);
  }

  export function sync(exec: ExecFn): void {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm -- schema:sync -d src/database/data-source.ts\`);
  }

  export function reset(exec: ExecFn): void {
    exec(\`${packageManagerCmds[flags.packageManager]} db:drop\`);
    exec(\`${packageManagerCmds[flags.packageManager]} migration:run\`);
...
export const migrationCliCmd = () => {
  ...
  export function run(exec: ExecFn) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm -- migration:run -d src/database/data-source.ts\`);
  }

  export function revert(exec: ExecFn) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm -- migration:revert -d src/database/data-source.ts\`);
  }

  export function show(exec: ExecFn) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm -- migration:show -d src/database/data-source.ts\`);
  }

  export function generate(exec: ExecFn, name: string) {
    exec(
      \`${packageManagerCmds[flags.packageManager]} typeorm -- migration:generate src/database/migrations/\${name} -d src/database/data-source.ts\`
    );
  }

  export function create(exec: ExecFn, name: string) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm -- migration:create src/database/migrations/\${name}\`);
  }`;
};
