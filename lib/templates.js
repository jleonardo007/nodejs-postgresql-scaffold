...
const packageManagerCmds = {
  npm: 'npm run',
  yarn: 'yarn',
  pnpm: 'pnpm',
};

export const databaseCliCmd = () => {
  const { flags } = useCliContext();
  return `type ExecFn = (command: string) => void;

  export function drop(exec: ExecFn): void {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} schema:drop -d src/database/data-source.ts\`);
  }

  export function sync(exec: ExecFn): void {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} schema:sync -d src/database/data-source.ts\`);
  }

  export function reset(exec: ExecFn): void {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} schema:drop -d src/database/data-source.ts\`); // Drop schema
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} schema:sync -d src/database/data-source.ts\`); // Run migrations to recreate schema
    exec(\`${packageManagerCmds[flags.packageManager]} seed\`);
  }`;
};

export const migrationCliCmd = () => {
  const { flags } = useCliContext();

  return `type ExecFn = (command: string) => void;

  export function run(exec: ExecFn) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} migration:run -d src/database/data-source.ts\`);
  }

  export function revert(exec: ExecFn) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} migration:revert -d src/database/data-source.ts\`);
  }

  export function show(exec: ExecFn) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} migration:show -d src/database/data-source.ts\`);
  }

  export function generate(exec: ExecFn, name: string) {
    exec(
      \`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} migration:generate src/database/migrations/\${name} -d src/database/data-source.ts\`
    );
  }

  export function create(exec: ExecFn, name: string) {
    exec(\`${packageManagerCmds[flags.packageManager]} typeorm ${flags.packageManager === 'pnpm' ? '' : '--'} migration:create src/database/migrations/\${name}\`);
  }`;
};

export const seedCliCmd = () => {
  const { flags } = useCliContext();
